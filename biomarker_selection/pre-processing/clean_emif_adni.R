...
# supp <- readxl::read_xlsx("_notgit/ADNI/awaa325-suppl_data/brain-2020-00570-File008.xlsx",
#                           skip = 2,
...
supp <- readxl::read_xlsx("/Users/felicia.burtscher/Documents/UL/DATASETS/Brain 2020 AD/awaa325-suppl_data/brain-2020-00570-File008.xlsx",
                          skip = 2,
                          sheet = "Supplementary table 2")[, c(1:6, 12:13)] %>%
  dplyr::filter(`Selected for clustering` == "Yes") %>%
  dplyr::rename(Name = Gene, UniProt = Uniprot, p.val = `p value...13`) %>%
...
write.table(dplyr::filter(final, Cohort == "ADNI"), file = "/Users/felicia.burtscher/Documents/UL/DATASETS/Brain 2020 AD/cleaned_ADNI_entries.tsv", 
            sep = "\t", quote = F, row.names = F, col.names = T)
write.table(dplyr::filter(final, Cohort == "EMIF-AD MBD"), file = "/Users/felicia.burtscher/Documents/UL/DATASETS/Brain 2020 AD/cleaned_EMIF_entries.tsv", 
            sep = "\t", quote = F, row.names = F, col.names = T)
